...
uint16_t DMA_Channel_Disable(enum dma_channel_number ch_num)
{
    if (ch_num >= DMA_CHANNEL_MAX)
        return DMA_INVALID_PARA;
    DMA_Num2Ch(ch_num)->CFG |= (1 << DMA_CFG_CH_SUSP_Pos) & DMA_CFG_CH_SUSP_Msk;
    while (!(DMA_Num2Ch(ch_num)->CFG & ((1 << DMA_CFG_FIFO_EMPTY_Pos) & DMA_CFG_FIFO_EMPTY_Msk)));
    DMA_EN->ChEn = DMA_CHEN_CH_WE_ENABLE << ch_num;
    return OK;
}
